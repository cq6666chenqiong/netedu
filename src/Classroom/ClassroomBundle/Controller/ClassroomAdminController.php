...
class ClassroomAdminController extends BaseController
{
    ...
    public function setAction(Request $request)
    {
        if ($request->getMethod() == 'POST') {
            $this->setFlashMessage('success', "科室设置成功！");
    ...
    public function addClassroomAction(Request $request)
    {
        if ($this->get('security.context')->isGranted('ROLE_ADMIN') !== true) {
            return $this->createMessageResponse('info', '目前只允许管理员创建科室!');
        }
        ...
        if ($request->getMethod() == 'POST') {
            ...
            if (empty($title)) {
                $this->setFlashMessage('danger', "科室名称不能为空！");
            ...
            if (!empty($isClassroomExisted)) {
                $this->setFlashMessage('danger', "科室名称已被使用，创建科室失败！");
            ...
            $classroom = array(
                ...
                'orgCode'  => '5'//$myClassroom['orgCode']
            );

            $classroom = $this->getClassroomService()->addClassroom($classroom);

            $this->setFlashMessage('success', "恭喜！创建科室成功！");
