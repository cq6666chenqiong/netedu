...
class ClassroomController extends BaseController
{
    ...
    /**
     * 如果用户已购买了此科室，或者用户是该科室的教师，则显示科室的Dashboard界面。
     * 如果用户未购买该科室，那么显示课程的营销界面。
     */
    ...
    public function exitAction(Request $request, $id)
    {
        ...
        if (empty($member)) {
            throw $this->createAccessDeniedException('您不是科室的学员。');
        }
    ...
    public function orderInfoAction(Request $request, $sn)
    {
        ...
        if (empty($classroom)) {
            throw $this->createNotFoundException("找不到要购买的科室!");
        }
