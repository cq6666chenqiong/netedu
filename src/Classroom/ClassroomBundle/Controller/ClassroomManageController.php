...
class ClassroomManageController extends BaseController
{
    ...
    public function menuAction($classroom, $sideNav)
    {
        ...
        if (!$user->isLogin()) {
            return $this->createErrorResponse($request, 'not_login', '用户未登录，创建科室失败。');
        }
    ...
    public function createAction(Request $request, $id)
    {
        ...
        if ('POST' == $request->getMethod()) {
            ...
            $classroomName    = isset($classroomSetting['name']) ? $classroomSetting['name'] : '科室';
            ...
            $this->getLogService()->info('classroom', 'add_student', "科室《{$classroom['title']}》(#{$classroom['id']})，添加学员{$user['nickname']}(#{$user['id']})，备注：{$data['remark']}");
    ...
    public function checkStudentAction(Request $request, $id)
    {
        ...
        if (!$user) {
            ...
            if ($isClassroomStudent) {
                $response = array('success' => false, 'message' => '该用户已是本科室的学员了');
            } else {
    ...
    public function exportCsvAction(Request $request, $id, $role)
    {
        ...
        $str = "用户名,Email,加入学习时间,学习进度,姓名,性别,QQ号,工号号,手机号,科室,职称,学位";
    ...
    public function excelDataImportAction(Request $request, $id)
    {
        ...
        if ($classroom['status'] != 'published') {
            throw $this->createNotFoundException("未发布科室不能导入学员!");
        }
