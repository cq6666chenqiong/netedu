...
class ClassroomServiceImpl extends BaseService implements ClassroomService
{
    ...
    public function addClassroom($classroom)
    {
        ...
        if (empty($title)) {
            throw $this->createServiceException('科室名称不能为空！');
        }
    ...
    //TO-DO 科室课程表加了一个父课程Id字段,逻辑可以重构了! BY wenqin 2015-5-21
...
//if new copy it
    ...
    public function deleteClassroom($id)
    {
        ...
        if (empty($classroom)) {
            throw $this->createServiceException('科室不存在，操作失败。');
        }

        if ($classroom['status'] != 'draft') {
            throw $this->createServiceException('只有未发布科室可以删除，操作失败。');
        }
        ...
        $this->getLogService()->info('Classroom', 'delete', "科室#{$id}永久删除");
    ...
    public function changePicture($id, $data)
    {
        ...
        if (empty($classroom)) {
            throw $this->createServiceException('科室不存在，图标更新失败！');
        }
    ...
    public function removeStudent($classroomId, $userId)
    {
        ...
        if (empty($member) || !(array_intersect($member['role'], array('student', 'auditor')))) {
            throw $this->createServiceException("用户(#{$userId})不是科室(#{$classroomId})的学员，退出科室失败。");
        }
        ...
        $this->getLogService()->info('classroom', 'remove_student', "科室《{$classroom['title']}》(#{$classroom['id']})，移除学员#{$member['id']}");
        $this->dispatchEvent(
            'classroom.quit',
    ...
    public function becomeStudent($classroomId, $userId, $info = array())
    {
        ...
        if (!in_array($classroom['status'], array('published', 'closed'))) {
            throw $this->createServiceException('不能加入未发布科室');
        }

        $user = $this->getUserService()->getUser($userId);

        if (empty($user)) {
            throw $this->createServiceException("用户(#{$userId})不存在，加入科室失败！");
        }

        $member = $this->getClassroomMember($classroomId, $userId);

        if (!$this->canBecomeClassroomMember($member)) {
            throw $this->createServiceException("该用户(#{$userId})不能成为该科室学员！");
        }
        ...
        if (!empty($info['becomeUseMember'])) {
            ...
            if ($levelChecked != 'ok') {
                throw $this->createServiceException("用户(#{$userId})不能以会员身份加入科室！");
            }
        ...
        if (!empty($info['orderId'])) {
            ...
            if (empty($order)) {
                throw $this->createServiceException("订单(#{$info['orderId']})不存在，加入科室失败！");
            }
        } else {
            $order = null;
        }
    ...
    public function becomeAuditor($classroomId, $userId)
    {
        ...
        if (!$this->canBecomeClassroomMember($member)) {
            throw $this->createServiceException("该用户(#{$userId})不能成为该科室的旁听生！");
        }
    ...
    public function exitClassroom($classroomId, $userId)
    {
        ...
        if (!$member) {
            throw $this->createAccessDeniedException('您不是科室学员，无法退出科室！');
        }

        if (!array_intersect($member['role'], array('student', 'auditor'))) {
            throw $this->createAccessDeniedException('教师无法退出科室！');
        }
    ...
    public function lockStudent($classroomId, $userId)
    {
        ...
        if (empty($classroom)) {
            throw $this->createNotFoundException("科室(#{$classroomId})不存在，封锁学员失败。");
        }

        $member = $this->getClassroomMember($classroomId, $userId);

        if (empty($member) || !in_array('student', $member['role'])) {
            throw $this->createServiceException("用户(#{$userId})不是科室(#{$classroomId})的学员，封锁学员失败。");
        }
    ...
    public function unlockStudent($classroomId, $userId)
    {
        ...
        if (empty($member) || !in_array('student', $member['role'])) {
            throw $this->createServiceException("用户(#{$userId})不是该科室(#{$classroomId})的学员，解封学员失败。");
        }
    ...
    public function recommendClassroom($id, $number)
    {
        ...
        if (!is_numeric($number)) {
            throw $this->createAccessDeniedException('推荐科室序号只能为数字！');
        }
        ...
        $this->getLogService()->info('classroom', 'recommend', "推荐科室《{$classroom['title']}》(#{$classroom['id']}),序号为{$number}");
    ...
    public function cancelRecommendClassroom($id)
    {
        ...
        $this->getLogService()->info('classroom', 'cancel_recommend', "取消推荐科室《{$classroom['title']}》(#{$classroom['id']})");
