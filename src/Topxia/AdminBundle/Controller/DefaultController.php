...
class DefaultController extends BaseController
{
    ...
    public function feedbackAction(Request $request)
    {
        ...
        $token = CurlToolkit::request('POST', "http://www.myBanana.com/question/get/token", array());
        $site  = array('name' => $site['name'], 'url' => $site['url'], 'token' => $token, 'username' => $user->nickname);
        $site  = urlencode(http_build_query($site));
        return $this->redirect("http://www.myBanana.com/question?site=".$site."");
    }
    ...
    {
        $url = "http://open.myBanana.com/api/v1/context/notice";
