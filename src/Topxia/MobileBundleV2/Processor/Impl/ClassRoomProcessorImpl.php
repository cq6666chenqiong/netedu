...
class ClassRoomProcessorImpl extends BaseProcessor implements ClassRoomProcessor
{
	public function after()
	{
		if (!class_exists('Classroom\Service\Classroom\Impl\ClassroomServiceImpl')) {
			$this->stopInvoke();
			return $this->createErrorResponse("no_classroom", "没有安装科室插件！");
		}
	}
    ...
    public function getTodaySignInfo()
    {
        ...
        $user  = $this->controller->getUserByToken($this->request);
        if (!$user->isLogin()) {
            return $this->createErrorResponse('not_login', "您尚未登录，不能查看科室！");
        }
	...
	public function exitClassRoom($classRoomId, $user)
	{
    		...
    		if (empty($member)) {
        		throw $this->createErrorResponse('error', '您不是科室的学员。');
    		}

    		if (!in_array($member["role"], array("auditor", "student"))) {
    			return $this->createErrorResponse('error', "您不是科室的学员。");
    		}
	...
	{	
        ...
        $user = $this->controller->getUserByToken($this->request);
        if (!$user->isLogin()) {
        	return $this->createErrorResponse('not_login', "您尚未登录，不能学习科室！");
    	}
	...
	public function getTeachers()
	{
        ...
        $classroom = $this->getClassroomService()->getClassroom($classRoomId);
        if (empty($classroom)) {
			return $this->createErrorResponse('error', "科室不存在!");
		}
    ...
    public function getClassRoomCourses()
    {
        ...
        $classroom = $this->getClassroomService()->getClassroom($classroomId);
        if (empty($classroom)) {
            return $this->createErrorResponse('error', "没有找到该科室");
        }
