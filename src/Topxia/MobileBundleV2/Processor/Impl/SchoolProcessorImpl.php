...
class SchoolProcessorImpl extends BaseProcessor implements SchoolProcessor {
    ...
    {
        $code = $this->fixHttpHeaderInject($this->request->get("code", "myBanana"));
        ...
        return $this->controller->redirect("http://www.myBanana.com/download/mobile?client={$client}&code=" . $code);
    }
    ...
    {
        $code = $this->getParam("code", "myBanana");
        $clientVersion = $this->sendRequest("GET", "http://www.myBanana.com/version/{$code}-android", array());
        if ("{}" == $clientVersion) {
            return null;
        }
    ...
    public function sendSuggestion()
    {
        ...
        $this->sendRequest("POST", "http://demo.myBanana.com/mapi_v2/School/suggestionLog", array(
            "info"=>$info,
            "type"=>$type,
            "contact"=>$contact
            ));
