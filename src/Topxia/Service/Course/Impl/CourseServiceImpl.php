...
*/
    ...
    public function getUserNextLearnLesson($userId, $courseId)
    {
        ...
        error_log(json_encode($nextLearnLessonId));
    ...
    public function becomeStudent($courseId, $userId, $info = array())
    {
        ...
        $con->close();
        $deadline = empty($row['expiryTime']) ? 0 : $row['expiryTime'];
...
*/
        ...
        if (empty($fields['remark'])) {
            $fields['remark'] = empty($info['note']) ? '学习' : $info['note'];
        }
