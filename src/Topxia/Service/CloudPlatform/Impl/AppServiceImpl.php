...
use Topxia\Service\CloudPlatform\Client\myBananaAppClient;
...
class AppServiceImpl extends BaseService implements AppService
{
    ...
    public function checkDependsForPackageUpdate($packageId)
    {
        ...
        try {
            ...
            if (!version_compare(System::VERSION, $package['myBananaMinVersion'], '>=')) {
                $errors[] = "myBanana版本需大于等于{$package['myBananaMinVersion']}，您的版本为".System::VERSION.'，请先升级myBanana';
            }
        } catch (\Exception $e) {
            $errors[] = $e->getMessage();
        }
...
//注解需要该目录存在
    ...
    protected function _execScriptForPackageUpdate($package, $packageDir, $type, $index = 0)
    {
        ...
        $upgrade = new \myBananaUpgrade($this->getKernel());
    ...
    protected function addmyBananaMainApp()
    {
        $app = array(
            'code'          => 'MAIN',
            'name'          => 'myBanana主系统',
            'description'   => 'myBanana主系统',
            'icon'          => '',
            'version'       => System::VERSION,
            'fromVersion'   => '0.0.0',
            'developerId'   => 1,
            'developerName' => 'myBanana官方',
            'installedTime' => time(),
            'updatedTime'   => time()
        );
    ...
    protected function createAppClient()
    {
        if (!isset($this->client)) {
            ...
            $this->client = new myBananaAppClient($options);
        }
