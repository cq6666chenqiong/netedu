...
class KeyApplier 
{
    ...
    public function applyKey($user, $edition = 'opensource', $source = 'apply')
    {
        ...
        $params['siteName'] = empty($site['name']) ? 'myBanana网络课程' : $site['name'];
        ...
        $url = empty($setting['cloud_api_server']) ? 'http://api.myBanana.net' : rtrim($setting['cloud_api_server'], '/');
        $url = $url . '/v1/keys';
    ...
    protected function postRequest($url, $params)
    {
        ...
        curl_setopt($curl, CURLOPT_USERAGENT, 'myBanana Install Client 1.0');
