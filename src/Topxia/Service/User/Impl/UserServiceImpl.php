...
/*增加用户*/
    public function addUser($registration){
        $user = array();
        ...
        $user['type']          = isset($registration['type']) ? $registration['type'] : 'default';
        ...
        if (in_array('default', array('default', 'phpwind', 'discuz'))) {
            $user['salt']     = base_convert(sha1(uniqid(mt_rand(), true)), 16, 36);
            $user['password'] = $this->getPasswordEncoder()->encodePassword($registration['password'], $user['salt']);
            $user['setup']    = 1;
        } elseif (in_array('default', array('qq', 'weibo', 'renren', 'weixinweb', 'weixinmob')) && isset($thirdLoginInfo["default_set_fill_account"]) && $thirdLoginInfo["{$type}_set_fill_account"]) {
            $user['salt']     = '';
            $user['password'] = '';
            $user['setup']    = 1;
        } else {
        ...
        $thirdLoginInfo = $this->getSettingService()->get('login_bind', array());
         ...
         $c = $this->getUserByEmail($user['email']);
        /*
...
*/
    ...
    {
        for ($i = 0; $i < $maxLoop; $i++) {
            $registration['email'] = 'user_'.substr($this->getRandomChar(), 0, 9).'@myBanana.net';
