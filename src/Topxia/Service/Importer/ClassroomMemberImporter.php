        ...
        foreach ($userData as $key => $user) {
            ...
            if ($isClassroomStudent || $isClassroomTeacher) {
                ...
                $order = $this->getOrderService()->createOrder(array(
                    'userId'     => $user['id'],
                    'title'      => "购买科室《{$targetObject['title']}》(管理员添加)",
                ...
                $this->getLogService()->info('classroom', 'add_student', "科室《{$targetObject['title']}》(#{$targetObject['id']})，添加学员{$user['nickname']}(#{$user['id']})，备注：通过批量导入添加");
            }
        }
