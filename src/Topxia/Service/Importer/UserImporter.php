...
error_log('ttttttttttttttttttttttttttttttttt1');
            if (!empty($value['classroomId'])) {
                ...
                foreach ($classroomIds as $classroomId) {
                    ...
                    error_log('ttttttttttttttttttttttttttttttttt2');
                    if ($mode = "update") {
                    ...
                    error_log('ttttttttttttttttttttttttttttttttt3');
                    //添加同时生成订单
                    $order = $this->getOrderService()->createOrder(array(
                        'userId'     => $user['id'],
                        'title'      => "购买科室《{$classroom['title']}》(管理员添加)",
                    ...
                    error_log('ttttttttttttttttttttttttttttttttt4');
                    $info = array(
                        'orderId' => $order['id'],
                        'note'    => ''
                    );
            ...
            error_log('ttttttttttttttttttttttttttttttttt5');
            if (!empty($value['courseId'])) {
                $courseIds = explode(",", $value['courseId']);
                error_log('ttttttttttttttttttttttttttttttttt6');
                foreach ($courseIds as $courseId) {
                    ...
                    error_log('ttttttttttttttttttttttttttttttttt7');
                    //添加同时生成订单
            ...
            if (!empty($userData['classroomId']) || $userData['classroomId'] == '0') {
                ...
                foreach ($classroomIds as $classroomId) {
                    ...
                    if ($isClassroomExit && $isClassroomExit['status'] != 'published') {
                        $errorInfo[] = "第" . $row . "行科室号为的" . $classroomId . "的科室未发布，请检查。";
                    } elseif (!$isClassroomExit) {
                        $errorInfo[] = "第" . $row . "行科室号为的" . $classroomId . "的科室不存在，请检查。";
                    }
                }
            }
            ...
            if (!empty($userData['courseId']) || $userData['courseId'] == '0') {
                ...
                foreach ($courseIds as $courseId) {
                    ...
                    if ($isCourseExit && $isCourseExit['status'] != 'published') {
                        ...
                        $errorInfo[] = "第" . $row . "行课程号为的" . $courseId . "的课程课程属于科室课程,无法加入";
                    } elseif (!$isCourseExit) {
