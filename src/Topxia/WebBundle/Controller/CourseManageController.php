...
class CourseManageController extends BaseController
{
    ...
    public function baseAction(Request $request, $id)
    {
        ...
        if ($request->getMethod() == 'POST') {
            ...
            $expiryTime = null;
            error_log(date('Y'));
            if(is_null($data['expiryTime'])){
                $expiryTime = strtotime(date('Y')." 23:59:59");
            }else{
                $expiryTime = strtotime($data['expiryTime']." 23:59:59");
            }
            ...
            $sql="select * from course_expriy where courseId = ".$id.";";
            error_log($sql);
            ...
            if(empty($result)){
                ...
                $expiryTime1 = strtotime($time);
                $sql="INSERT INTO `course_expriy` (`expiryTime`, `courseId`) VALUES ( '".$expiryTime1."', '".$id."');";
                error_log($sql);
                mysqli_select_db($con,System::$DBNAME);
                mysqli_multi_query($con,"set names 'utf8'");
                $result = mysqli_query($con,$sql);
            }else {
                $sql="update `course_expriy` set `expiryTime` = '".$expiryTime."' where courseId = '".$id."';";
                error_log($sql);
                mysqli_select_db($con,System::$DBNAME);
                mysqli_multi_query($con,"set names 'utf8'");
                $result = mysqli_query($con,$sql);
        ...
        error_log($row['expiryTime']);
        error_log(date("Y-m-d",$row['expiryTime']));
        $course['expiryTimeStr'] = date("Y-m-d",$row['expiryTime']);
        //print_r($course);
        $con->close();
    ...
    public function orderExportCsvAction(Request $request, $id)
    {
        ...
        $payment = array('alipay' => '支付宝', 'wxpay' => '工号支付', 'cion' => '虚拟币支付', 'none' => '--');
