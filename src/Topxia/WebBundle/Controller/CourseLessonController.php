...
//课时不并且不满足1.有时间限制设置2.课时为视频课时3.视频课时非优酷等外链视频时提示购买
...
// 如果支持wml和html但是wml在html之前则是移动设备
    ...
    public function listAction(Request $request, $courseId, $member, $previewUrl, $mode = 'full')
    {
        ...
        $finishLessonItem = array();
        foreach ($learnStatuses as $key => $value) {
            if($value == 'finished'){
                array_push($finishLessonItem,$key);
            }
        }
        ...
        return $this->Render('TopxiaWebBundle:CourseLesson/Widget:list.html.twig', array(
            ...
            'finishLessonItem'         => $finishLessonItem
