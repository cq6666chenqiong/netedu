...
class CourseController extends CourseBaseController
{
    ...
    public function infoAction(Request $request, $id)
    {
        ...
        if ($course['parentId']) {
            ...
            if (!$this->getClassroomService()->canLookClassroom($classroom['classroomId'])) {
                return $this->createMessageResponse('info', '非常抱歉，您无权限访问该科室，如有需要请联系客服', '', 3, $this->generateUrl('homepage'));
            }
        }
    ...
    public function createAction(Request $request)
    {
        ...
        if ($type == 'live') {
            ...
            if (empty($capacity['capacity']) && !empty($courseSetting['live_course_enabled'])) {
                return $this->createMessageResponse('info', '请联系myBanana官方购买直播教室，然后才能开启直播功能！');
            }
        }
        ...
        if ($request->getMethod() == 'POST') {
            ...
            $sql = "select * from course_expriy where courseId = ".$course['id'].";";

            $result = System::getOneResult($con,$sql);
            if(empty($result)){
                $year=intval (date("Y"));
                $time = $year."-12-31 23:59:59";
                $expiryTime = strtotime($time);
                $sql = "INSERT INTO `course_expriy` ( `expiryTime`, `courseId`) VALUES ( '".$expiryTime."', '".$course['id']."');";
                System::excuteSql($con,$sql);
            }
    ...
    public function learnAction(Request $request, $id)
    {
        ...
        $sql = "select id,mediaSource,mediaUri,ifnull(length,0) length,userId,seq  from course_lesson where id = ".$lessonId.";";
        error_log($sql);
        ...
        $sql = "select id from course_lesson where status = 'published' and courseId = ".$id." and seq > ".$lesson['seq'] ." order by seq asc limit 0,1;"; //pdatedTime
        error_log($sql);
        mysqli_select_db($con,System::$DBNAME);
        mysqli_multi_query($con,"set names 'utf8'");
        $result = mysqli_query($con,$sql);
        $nextLearnLesson = mysqli_fetch_array($result);
        if($nextLearnLesson == null){
            $nextLearnLesson = array();
            $nextLearnLesson['id'] = -1;
        }
        $con->close();
        error_log(json_encode($nextLearnLesson));
